...
describe('Retrive User Post', () => {
    context('GET /public/v2/users/${userId}/posts', () => {

        it('Retrive User Post', ()=>{
    ...
    cy.request({
        ...
        //cy.log(res);
        expect(response).to.have.property('status').to.equal(200)
        cy.task('log', 'Retrieved post for user: ' + userId)
    
    })
})
})
})
describe('Retrives Post comments', () => {
    context('GET /public/v2/posts/${userId}/comments', () => {

        it('Retrives Post Comments', ()=>{

    cy.request({

        method:'GET',
        url:'/public/v2/posts/${userId}/comments'

    }).then((response) => {
        let res = JSON.stringify(response.body);
        //cy.log(res);
        expect(response).to.have.property('status').to.equal(200)
        cy.task('log', 'Retrieved post comments for user: ' + userId)
    })
})
})
})
